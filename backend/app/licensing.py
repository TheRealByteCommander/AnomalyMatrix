...
class LicenseManager:
    ...
    def activate(self, key: str, *, seats_requested: int, device_id: str) -> LicenseSnapshot:
        now = datetime.now(timezone.utc)
        key = key.strip()
        ...
        if key.startswith('AMX-EXPIRED'):
            ...
            features = {'dashboard_run': True, 'inspection_detail': True, 'trends_filters': False, 'advanced_export': False}
            message = 'License expired, grace active'
        elif key.startswith('AMX-OFFLINE'):
            state = 'offline'
            expires = now + timedelta(days=14)
            grace = now + timedelta(days=2)
            features = {'dashboard_run': True, 'inspection_detail': True, 'trends_filters': True, 'advanced_export': False}
            message = 'Offline validation mode'
        elif len(key) >= 8:
            state = 'active'
            expires = now + timedelta(days=365)
            grace = None
            features = {'dashboard_run': True, 'inspection_detail': True, 'trends_filters': True, 'advanced_export': True}
